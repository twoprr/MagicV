...
if (str_contains($s, 'location_patch.css')) {
    echo "OK: location_patch.css already connected\n";
    exit(0);
}

$bak = $layout . '.bak-location-ui-' . date('YmdHis');
copy($layout, $bak);

$line = '<link href="/assets/css/location_patch.css" rel="stylesheet">';
if (str_contains($s, '<link href="/assets/css/home_legal_patch.css" rel="stylesheet">')) {
    $s = str_replace(
